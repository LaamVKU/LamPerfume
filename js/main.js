...
$(function () {
    ...
    // get so luong va luu vao localstorage, sau do hien thi so luong
    ...
    function setItem(product) {
        ...
        if (cartItems != null) {
            ...
            cartItems[product.name].inCart += parseInt($(".soluongsp").val());
    ...
    // tinh tien va luu vao localstorage
    function totalCost(product) {
        let cartCost = localStorage.getItem('totalCost');

        if (cartCost != null) {
            cartCost = parseFloat(cartCost);
            localStorage.setItem('totalCost', cartCost + parseInt($(".soluongsp").val()) * 1000 *  product.price );
        } else {
            localStorage.setItem('totalCost', parseInt($(".soluongsp").val()) * product.price *1000);
        }
    }
    ...
    $(".btn-checkout").click(function (e) {
        ...
        alert("Thanh toán thành công, Cảm ơn đã mua hàng");
    });
...
window.onload = () => {
    ...
    signup.onclick = (e) => {
        ...
        if(localStorage.user){
            let checkList = JSON.parse(localStorage.user)
            let check = Array.from(checkList).find(item => item.email == obj.email)
            if(!check){
                userAccount.push(obj)
                localStorage.user = JSON.stringify(userAccount)
    ...
    signin.onclick = (e) => {
        ...
        if(localStorage.user){
            let checkList = JSON.parse(localStorage.user)
            console.log(passSignin.value, emailSignin.value);
            console.log('checkList', checkList);
            let check = Array.from(checkList).find(item => (item.email == emailSignin.value && item.password == passSignin.value))
            console.log('check:' +check);
            if(!check){
                
                alert('Email hoặc mật khẩu không đúng')
            } else{
                document.querySelector('.nav-item .nav-link').textContent = check.name
